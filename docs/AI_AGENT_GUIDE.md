...
This project ships a single Axum service that ingests camera timelines, runs Whisper-based transcription, and exposes an HTML/JSON interface for reviewing results. The code is organised so automated refactors can focus on a few well-defined surfaces. Use this guide as a quick orientation for large-language-model powered tooling.
...
2. `AppState` holds pooled connections to three SQLite databases, background statistics, caches, OpenAI client handles, and configuration such as Whisper URL and timezone.
...
| `zumblezay_db` | Application state and cached transcriptions. | `events`, `transcriptions`, `daily_summaries`, `corrupted_files`. |
...
- Summaries: `/api/transcripts/summary/...` including model/prompt selection.
...
3. Successful transcripts call `transcripts::save_transcript`, which inserts a `transcriptions` row and updates stats.
